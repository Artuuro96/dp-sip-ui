...
export default function DashboardAppPage() {
  ...
  const getAnalyticsSales = async () => {
    setLoading(true);
    setContext(userInfo);
    try {
      const res = await promerClient.getAnalyticsSales();
      setSaleStats(res);
      setLoading(false);
    } catch (error) {
      setLoading(false);
    ...
    <>
      ...
      <Container maxWidth="xl">
        <Typography variant="h4" sx={{ mb: 5 }}>
          Hola {context.name} {context.lastName}!
        </Typography>

        <Grid container spacing={3}>
          <Grid item xs={12} sm={6} md={4}>
            <AppWidgetSummary title="Ventas Totales" total={fCurrency(saleStats?.total)} icon={'iconoir:home-sale'} />
          </Grid>
          ...
          <Grid item xs={12} sm={6} md={4}>
            <AppWidgetSummary title="Terrenos Vendidos" total={saleStats?.totalSales} color="success" icon={'mdi:land-plots-marker'} />
          </Grid>

          {/* <Grid item xs={12} sm={6} md={3}>
            <AppWidgetSummary title="Total de Ventas" total={'0'} icon={'carbon:rule-draft'} />
  </Grid> */}
